...
export const getSessionId =  async (token, propertyId) => await axios.post(`${process.env.REACT_APP_API}/stripe-session-id`, {propertyId}, {
    headers: {
        Authorization: `Bearer ${token}`,
    }
})

export const stripeSuccessRequest =  async (token, propertyId) => await axios.post(`${process.env.REACT_APP_API}/stripe-success`, {propertyId}, {
    headers: {
        Authorization: `Bearer ${token}`,
    }
});